...
class DatabaseService {
    ...
    async connect() {
        try {
            ...
            logger_1.scraperLogger.info('Connected to EdgeDB successfully');
        }
        catch (error) {
            logger_1.scraperLogger.error('Failed to connect to EdgeDB', error);
            throw error;
        }
    }
    async disconnect() {
        try {
            ...
            logger_1.scraperLogger.info('Disconnected from EdgeDB');
        }
        catch (error) {
            logger_1.scraperLogger.error('Error disconnecting from EdgeDB', error);
        }
    }
    // Tweet operations
    ...
    async getTweets(limit = 20, _offset = 0, username, includeRetweets = true, includeReplies = true) {
        try {
            let query = `
        SELECT Tweet {
          tweetId,
    ...
    async getTweetById(tweetId) {
      ...
      `;
            ...
            logger_1.scraperLogger.error(`Failed to get tweet ${tweetId}`, error);
            return null;
        }
    }
    ...
    async getUser(username) {
      ...
      `;
            ...
            logger_1.scraperLogger.error(`Failed to get user ${username}`, error);
            return null;
        }
    }
