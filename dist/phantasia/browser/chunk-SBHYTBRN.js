...
// node_modules/.pnpm/@angular+animations@20.1.4_@angular+common@20.1.4_@angular+core@20.1.4_@angular+compile_e49daafdf390efbc8f556d12c468dd41/node_modules/@angular/animations/fesm2022/animations.mjs
...
// src/app/services/artist-credit.service.ts
var _ArtistCreditService = class _ArtistCreditService {
  constructor(http) {
    this.http = http;
    this.tracksWithCreditsSubject = new BehaviorSubject([]);
    this.currentTrackCreditsSubject = new BehaviorSubject(null);
    this.verificationStatusSubject = new BehaviorSubject([]);
    this.currentProjectSubject = new BehaviorSubject("phantasia1");
    this.projectMetadataSubject = new BehaviorSubject([]);
    this.allProjectTracksSubject = new BehaviorSubject([]);
    this.tracksWithCredits$ = this.tracksWithCreditsSubject.asObservable();
    this.currentTrackCredits$ = this.currentTrackCreditsSubject.asObservable();
    this.verificationStatus$ = this.verificationStatusSubject.asObservable();
    this.currentProject$ = this.currentProjectSubject.asObservable();
    this.projectMetadata$ = this.projectMetadataSubject.asObservable();
    this.allProjectTracks$ = this.allProjectTracksSubject.asObservable();
    this.completeArtistDatabase = {
      "SpiralFlip": {
  ...
  getArtistData(artistName) {
    ...
    return __spreadProps(__spreadValues({}, artistData), {
  ...
  getVerificationReport() {
    return this.verificationStatus$.pipe(map((verifications) => {
      const totalTracks = verifications.length;
      const fullyVerified = verifications.filter((v) => v.fullyVerified).length;
      const partiallyVerified = verifications.filter((v) => !v.fullyVerified && (v.allContributorsIdentified || v.rolesAccuratelyAssigned)).length;
    ...
    allTracks.forEach((track) => {
      track.allContributions.forEach((contribution) => {
        ...
        if (!allArtists.has(key)) {
          allArtists.set(key, __spreadProps(__spreadValues({}, contribution), {
            id: key
          }));
        }
      });
  ...
  getCrossProjectArtists() {
    ...
    const crossProjectNames = Array.from(phantasia1Names).filter((name) => phantasia2Names.has(name));
    return phantasia2Artists.filter((artist) => crossProjectNames.includes(artist.artistName));
  }
  /**
   ...
   */
  createArtistContribution(artistName, role, participationType, percentage, notes) {
    const artistData = this.completeArtistDatabase[artistName];
    if (!artistData) {
   ...
   */
  initializeAllProjectTracks() {
    const phantasia2Tracks = this.createCompleteTrackCredits().map((track) => __spreadProps(__spreadValues({}, track), {
      projectId: "phantasia2",
  ...
  createPhantasia1TrackCredits() {
    const trackCredits = [
      ...
      {
        id: "p1-5",
        title: "Rohkeutta Etsi\xE4",
        trackNumber: 5,
        startTime: 720,
        endTime: 900,
        audioFile: "05. Prower - Rohkeutta Etsi\xE4.ogg",
        projectId: "phantasia1",
        projectDisplayName: "Phantasia Project 1",
...
_ArtistCreditService.\u0275fac = function ArtistCreditService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _ArtistCreditService)(\u0275\u0275inject(HttpClient));
};
_ArtistCreditService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _ArtistCreditService, factory: _ArtistCreditService.\u0275fac, providedIn: "root" });
var ArtistCreditService = _ArtistCreditService;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(ArtistCreditService, [{
    type: Injectable,
    args: [{
      providedIn: "root"
    }]
...
@angular/animations/fesm2022/animations.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-SBHYTBRN.js.map
