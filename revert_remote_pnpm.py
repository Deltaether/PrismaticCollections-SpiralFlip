...
def revert_remote_to_pnpm():
    try:
        ...
        print("🔄 REVERTING REMOTE SCRIPTS BACK TO PNPM PACKAGE MANAGER")
        print("=" * 65)
        
        # Revert deployment script to reference pnpm instead of uv
        reverted_script = '''#!/bin/bash
...
if [ -d "dist/phantasia/browser" ]; then
    ...
    echo "   📦 Package Manager: PNPM (for local development only)"
...
if curl -s http://localhost/ | grep -i "phantasia\\|html\\|angular\\|collections" > /dev/null 2>&1; then
    ...
    echo "📦 Package Manager: PNPM (local development only)"
...
echo "============================================"
echo "📊 Pre-built deployment (PNPM-based) finished at $(date)"
'''
        
        # Write the reverted script to all deployment script locations
        sftp = ssh.open_sftp()
        try:
            with sftp.open('/root/deploy-phantasia.sh', 'w') as f:
                f.write(reverted_script)
            print("✅ Reverted /root/deploy-phantasia.sh to PNPM references")
            
            with sftp.open('/root/deploy-prebuilt.sh', 'w') as f:
                f.write(reverted_script)
            print("✅ Reverted /root/deploy-prebuilt.sh to PNPM references")
            
            with sftp.open('/root/deploy_from_github.sh', 'w') as f:
                f.write(reverted_script)
            print("✅ Reverted /root/deploy_from_github.sh to PNPM references")
        ...
        print("\n🎯 All deployment scripts reverted back to PNPM package manager references!")
...
if __name__ == "__main__":
    ...
    if revert_remote_to_pnpm():
        print("""
🎉 REMOTE SCRIPTS REVERTED TO PNPM!

✅ Changes Applied:
• Reverted deployment script comments to reference PNPM
• Instructions now show 'pnpm run build' instead of 'uv run build'  
• Package manager references reverted to PNPM
...
🚀 Server deployment workflow remains the same, now PNPM-aligned again!
        """)
    else:
        print("❌ Revert failed")