 ...
 * Includes video background, music player integration, two-state scroll system, and 16+ artists
 */
@Component({
  selector: 'app-phantasia1',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './phantasia1.html',
  styleUrls: ['./phantasia1.scss'],
...
export class Phantasia1Component implements OnInit, OnDestroy {
  ...
  @HostBinding('class.phantasia1-component') componentClass = true;
  ...
  ngOnInit(): void {
    if (this.isDebugMode) {
      console.log(`[Phantasia1Component] Current URL: ${this.router.url}`);
    }
    ...
    if (typeof document !== 'undefined') {
      ...
      if (this.isDebugMode) {
        console.log(`[Phantasia1Component] Added phantasia-album-page class to body`);
      }
    }
  ...
  get isInsidePhantasiaLayout(): boolean {
    // The URL will be /phantasia/phantasia1 when inside the layout
    ...
    this.videoElement.addEventListener('ended', () => {
      ...
      if (!this.autoplayFailed || this.userHasInteracted) {
        this.videoElement.play().then(() => {
          ...
          if (this.isDebugMode) {
            console.warn('[Phantasia1Component] Video replay failed:', error);
          }
        });
      }
    });
  ...
  private attemptVideoPlay(): void {
    ...
    if (playPromise !== undefined) {
      playPromise.then(() => {
        ...
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Video autoplay succeeded');
        }
        this.fadeInVideo();
      }).catch(error => {
        ...
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Video autoplay failed (expected):', error.name);
          console.log('[Phantasia1Component] Video will start after user interaction');
        }
  ...
  private setupUserInteractionDetection(): void {
    const handleFirstInteraction = () => {
      if (!this.userHasInteracted) {
        ...
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] User interaction detected');
        }
  ...
  private attemptVideoPlayAfterInteraction(): void {
    ...
    this.videoElement.play().then(() => {
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Video started after user interaction');
      }
      this.fadeInVideo();
    }).catch(error => {
      if (this.isDebugMode) {
        console.warn('[Phantasia1Component] Video play failed even after user interaction:', error);
      }
    });
  }
  ...
  private startLoadingSequence(): void {
    ...
    const progressInterval = setInterval(() => {
      ...
      if (this.loadingProgress >= 100) {
        ...
        setTimeout(() => {
          ...
          if (this.isDebugMode) {
            console.log('[Phantasia1Component] Loading complete, showing main content');
          }
        }, 500);
      }
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Loading sequence started');
    }
  }
  ...
  onTitleVideoLoaded(): void {
    if (this.titleVideo?.nativeElement) {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Title video loaded');
      }
    }
  }
  ...
  private attemptTitleVideoPlay(): void {
    ...
    if (playPromise !== undefined) {
      playPromise.then(() => {
        ...
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Title video autoplay succeeded');
        }
      }).catch(error => {
        this.titleAutoplayFailed = true;
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Title video autoplay failed (expected):', error.name);
        }
      });
    }
  }
  ...
  private attemptTitleVideoPlayAfterInteraction(): void {
    ...
    this.titleVideoElement.play().then(() => {
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Title video started after user interaction');
      }
    }).catch(error => {
      if (this.isDebugMode) {
        console.warn('[Phantasia1Component] Title video play failed even after user interaction:', error);
      }
    });
  }
  ...
  onTitleVideoEnded(): void {
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Title video playback completed - keeping video visible');
    }
  }
  ...
  private setupSmoothScrolling(): void {
    ...
    if (typeof document !== 'undefined') {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Smooth scrolling enabled globally');
      }
    }
  }
  ...
  private initializeTwoStateScroll(): void {
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Two-state scroll system initialized');
      console.log('[Phantasia1Component] Initial scroll state:', this.scrollState);
    }
  }
  ...
  private updateScrollState(): void {
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Scroll state updated:', this.scrollState, 'at position:', currentScrollY);
    }
  }
  ...
  private scrollToTop(): void {
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Auto-scrolling to top');
    }
    ...
    setTimeout(() => {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Arrived at top, scroll unlocked');
      }
    }, 1000);
  }
  ...
  private resetToTop(): void {
    if (typeof window !== 'undefined') {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Reset to top position');
      }
    }
  }
  ...
  scrollToContent(): void {
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Auto-scrolling to content section');
    }
    ...
    if (albumPresentation) {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Using album presentation position:', albumPresentation.offsetTop);
      }
    } else if (cdShowcase) {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Using calculated CD showcase absolute position:', offsetTop);
      }
    } else {
      // Final fallback: use viewport height to get past video section
      targetPosition = window.innerHeight - 90;
      if (this.isDebugMode) {
        console.warn('[Phantasia1Component] Using fallback viewport-based position');
      }
    }
    ...
    setTimeout(() => {
      ...
      if (this.isDebugMode) {
        console.log('[Phantasia1Component] Arrived at content, scroll unlocked');
      }
    }, 1200);
  }
  ...
  private hideScrollIndicatorAfterUse(): void {
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Scroll indicator hidden after use');
    }
  }
  ...
  private initializeDynamicArtistSystem(): void {
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Initializing dynamic artist system');
    }
   ...
   * Default: showcase mode (all 16+ Phantasia1 artists)
  ...
  private setupArtistDisplayModeLogic(): void {
    ...
    this.audioService.audioState$.subscribe((audioState: AudioState) => {
      if (audioState.isPlaying && audioState.currentTrack) {
        ...
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Switched to currently-playing mode for track:', audioState.currentTrack);
        }
      } else {
        // Switch back to showcase mode when music stops or no track
        this.artistDisplayMode.set('showcase');
        if (this.isDebugMode) {
          console.log('[Phantasia1Component] Switched to showcase mode (music stopped or no track)');
        }
      }
      this.cdr.markForCheck();
    });
    ...
    if (this.isDebugMode) {
      console.log('[Phantasia1Component] Artist display mode logic initialized - starting in showcase mode');
    }
  }
  ...
  ngOnDestroy(): void {
    ...
    if (typeof document !== 'undefined') {
      ...
      if (this.isDebugMode) {
        console.log(`[Phantasia1Component] Removed phantasia-album-page class from body`);
      }
    }
    ...
    if (this.isDebugMode) {
      console.log(`[Phantasia1Component] Component destroyed, cleanup completed`);
    }
  }
}