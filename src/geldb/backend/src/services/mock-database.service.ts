...
export class MockDatabaseService {
  ...
  private users: Map<string, TwitterUser> = new Map();
  ...
  async connect(): Promise<void> {
    scraperLogger.info('Connected to Mock Database successfully');
  }

  async disconnect(): Promise<void> {
    scraperLogger.info('Disconnected from Mock Database');
  }
  ...
  async saveTweet(tweet: Tweet): Promise<boolean> {
    try {
      ...
      scraperLogger.debug(`Tweet ${tweet.tweetId} saved to mock database`);
      return true;
    } catch (error) {
  ...
  async saveUser(user: TwitterUser): Promise<boolean> {
    try {
      ...
      scraperLogger.debug(`User ${user.username} saved to mock database`);
      return true;
    } catch (error) {
  ...
  private getProfileImageUrl(username: string): string {
  ...
  private loadMockData(): void {
    ...
    scraperLogger.info(`Loaded ${mockTweets.length} mock tweets and ${this.users.size} mock users`);
  }
}