...
import fs from 'fs';
import { Tweet, TwitterUser, RawTweetData, ScrapingOptions } from '../models/tweet.model';
...
export class TwitterScraperService {
  ...
  async initialize(): Promise<void> {
    try {
      scraperLogger.info('Initializing Twitter scraper with persistent browser data');
      ...
      scraperLogger.info('Waiting 1 minute for browser stabilization...');
      await new Promise(resolve => setTimeout(resolve, 60000)); // 60 seconds
      scraperLogger.info('Browser stabilization wait completed');
      ...
      scraperLogger.info('Twitter scraper initialized successfully');
    } catch (error) {
      scraperLogger.error('Failed to initialize Twitter scraper', error);
      throw error;
    }
  }

  async scrapeUserProfile(username: string): Promise<TwitterUser | null> {
    if (!this.page) {
      throw new Error('Scraper not initialized');
    }

    try {
      scraperLogger.info(`Scraping user profile: ${username}`);
      ...
      scraperLogger.info(`User profile scraped: ${username}`, {
      ...
      scraperLogger.error(`Failed to scrape user profile: ${username}`, error);
      return null;
    }
  }

  async scrapeTweets(username: string, options: ScrapingOptions = {}): Promise<Tweet[]> {
    if (!this.page) {
      throw new Error('Scraper not initialized');
    }
    ...
    try {
      const {
        ...
        includeReplies = true,
        delayMin = 180000,  // 3 minutes
        delayMax = 210000   // 3.5 minutes
      } = options;

      scraperLogger.info(`Starting tweet scraping for ${username}`, {
        maxTweets,
        includeRetweets,
        includeReplies
      });
      ...
      while (processedTweets < maxTweets && consecutiveEmptyScrolls < maxEmptyScrolls && this.isRunning) {
        ...
        for (const rawTweet of rawTweets) {
          if (processedTweets >= maxTweets) break;
          if (seenTweetIds.has(rawTweet.id)) continue;

          // Apply filters
          if (!includeRetweets && rawTweet.isRetweet) continue;
          if (!includeReplies && rawTweet.isReply) continue;
          ...
          scraperLogger.debug(`Processed tweet ${rawTweet.id}`, {
        ...
        if (newTweetsFound === 0) {
          ...
          scraperLogger.info(`Waiting additional ${(additionalDelay/1000).toFixed(1)} seconds after processing ${newTweetsFound} tweets`);
      ...
      scraperLogger.info(`Tweet scraping completed for ${username}`, {
      ...
      scraperLogger.error(`Failed to scrape tweets for ${username}`, error);
      throw error;
    } finally {
  ...
  private async extractTweetsFromPage(): Promise<RawTweetData[]> {
    if (!this.page) return [];
    ...
    return await this.page.evaluate(() => {
      ...
      tweetElements.forEach((tweetEl) => {
        try {
          ...
          // Extract avatar
          const avatarEl = tweetEl.querySelector('[data-testid="Tweet-User-Avatar"] img') as HTMLImageElement;
          ...
          const media = Array.from(mediaElements).map((el) => ({
            type: el.tagName.toLowerCase() === 'img' ? 'image' as const : 'video' as const,
  ...
  private convertRawTweetToTweet(rawTweet: RawTweetData): Tweet {
    return {
      tweetId: rawTweet.id,
      text: rawTweet.text,
  ...
  async saveScrapedData(tweets: Tweet[], user?: TwitterUser): Promise<void> {
    try {
      scraperLogger.info(`Saving ${tweets.length} tweets to database`);

      // Save user if provided
      if (user) {
        await this.database.saveUser(user);
        scraperLogger.info(`User ${user.username} saved to database`);
      }
      ...
      scraperLogger.info(`Saved ${savedCount}/${tweets.length} tweets to database`);
    } catch (error) {
      scraperLogger.error('Failed to save scraped data', error);
      throw error;
    }
  }
  ...
  async isLoggedIn(): Promise<boolean> {
    ...
    try {
      ...
      scraperLogger.warn('Could not determine login status', error);
      return false;
    }
  }
  ...
  async stop(): Promise<void> {
    this.isRunning = false;
    scraperLogger.info('Scraper stop requested');
  }
  ...
  async close(): Promise<void> {
    try {
      ...
      scraperLogger.info('Twitter scraper closed');
    } catch (error) {
      scraperLogger.error('Error closing scraper', error);
    }
  }
